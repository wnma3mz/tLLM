...
class RPCServicer(schemas_pb2_grpc.RPCServiceServicer):
    ...
    def InitModel(self, request, context):
        if self.init_model_flag:
            return schemas_pb2.StatusResponse(
                msg="Model already initialized", status=200
            )
        ...
        logging.info(
            f"{self.prefix_log_str} Model initialized cost time: {cost_time:.2f} s"
        )
    ...
    def InitMLP(self, request, context):
        ...
        s1 = time.time()
        logging.info(
            f"{self.prefix_log_str} Init MLP {data.proj_name} Config: {data.input_size}x{data.output_size}"
        )
        try:
            ...
            return schemas_pb2.StatusResponse(
                msg="MLP initialization failed", status=500
            )
        logging.info(
            f"{self.prefix_log_str} MLP {data.name} initialized cost time: {time.time() - s1:.2f} s"
        )
    ...
    def ForwardMLP(self, request, context):
        ...
        cost_time = time.time() - s1
        logging.info(
            f"{self.prefix_log_str} Forward MLP {data.name} cost time: {cost_time} s"
        )
        return schemas_pb2.ForwardResponse(
    ...
    def MLPKeys(self, request, context):
        return schemas_pb2.MLPKeysResponse(
            msg=str(list(self.mlp_dict.keys())), status=200
        )
