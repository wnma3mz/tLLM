...
class FeedForward:
    ...
    def load_state_dict(self, state_dict: dict):
        self.up_weight = state_dict["up_proj"].T
        self.gate_weight = state_dict["gate_proj"].T
        self.down_weight = state_dict["down_proj"].T
...
class Attention:
    def __init__(
        ...
        # max_batch_size = 1
        # self.cache_k = np.zeros((max_batch_size, args.max_position_embeddings, self.n_local_kv_heads, self.head_dim))
        # self.cache_v = np.zeros((max_batch_size, args.max_position_embeddings, self.n_local_kv_heads, self.head_dim))
    ...
    def __call__(
        ...
        # RoPE #2
        xq, xk = apply_rotary_emb(xq, xk, freqs_cos_sin)
        ...
        # logging.info(f"xk: {xk.shape}, xv: {xv.shape}, self.cache_k[:B, : start_pos + L]: {self.cache_k[:B, : start_pos + L].shape}, self.cache_v[:B, : start_pos + L]: {self.cache_v[:B, : start_pos + L].shape}")
    ...
    def load_state_dict(self, state_dict: dict):
        self.q_weight = state_dict["q_proj"].T
        self.k_weight = state_dict["k_proj"].T
        self.v_weight = state_dict["v_proj"].T
        self.o_weight = state_dict["o_proj"].T


class TransformerBlock:
    def __init__(self, args, layer_idx: int):
        self.layer_idx = layer_idx
        self.attention = Attention(None, None, None, None, args, layer_idx)
        self.mlp = FeedForward(None, None, None)
        self.input_layernorm = RMSNorm(None, eps=args.rms_norm_eps)
        self.post_attention_layernorm = RMSNorm(None, eps=args.rms_norm_eps)

    def __call__(
        self, x: Array["B, L or 1, D"], start_pos: int, mask: Array["L, L"], freqs_cos_sin, past_key_value=None
    ):
        # RMSNorm
        norm_x: Array["B, L or 1, D"] = self.input_layernorm(x)
        # Masked Multi-Head Attention
        h1, past_key_value = self.attention(norm_x, start_pos, mask, freqs_cos_sin, past_key_value)
        z = x + h1

        # RMSNorm
        norm_z = self.post_attention_layernorm(z)
        # Feed Forward + SwiGLU
        h2: Array["B, L or 1, D"] = self.mlp(norm_z)
        out = z + h2

        return out, past_key_value

    def load_state_dict(self, state_dict: dict):
        self.input_layernorm.load_state_dict({"weight": state_dict["input_layernorm.weight"]})
        self.post_attention_layernorm.load_state_dict({"weight": state_dict["post_attention_layernorm.weight"]})
        self.mlp.load_state_dict(
            {key: state_dict[f"mlp.{key}.weight"] for key in ["up_proj", "gate_proj", "down_proj"]}
        )
        self.attention.load_state_dict(
            {key: state_dict[f"self_attn.{key}.weight"] for key in ["q_proj", "k_proj", "v_proj", "o_proj"]}
        )


class Embedding:
    def __init__(self, weight: Array["VS, D"]):
        self.weight = weight

    def __call__(self, input_ids: Array["B, L"]):
...
class Linear:
    def __init__(self, weight: Array["D, VS"]):
        self.weight = weight

    def __call__(self, x: Array["B, L, D"]):
        return x @ self.weight

    def load_state_dict(self, state_dict: dict):
        self.weight = state_dict["weight"].T
...
#         # RoPE #1
#         self.freqs_cos, self.freqs_sin = compute_cos_sin_cache(args.dim // args.n_heads, args.max_position_embeddings)

#         self.layers = []
#         for layer_id in range(args.n_layers):
#             self.layers.append(TransformerBlock(weight, layer_id, args))

#         self.norm = RMSNorm(weight.get("model.norm.weight"), eps=args.rms_norm_eps)
